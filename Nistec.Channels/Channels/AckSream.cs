...
namespace Nistec.Channels
{
    ...
    public static class AckStreamExtension
    {
        public static object ReadAckValue(this AckStream ack, TransformType returnType)
        {
            ack.ReadAck(returnType);
            return ack.Value;
        }
        //public static int PeekFirstInt(this NetStream ack)
        //{
        //   return ack.PeekInt32(0);
        //}
    }
    ...
    public class AckStream : NetStream, IDisposable
    {
        ...
        public static AckStream GetAckStream(MessageState state, string action)
        {
            return new AckStream(state, action);
        }
        public static AckStream GetAckValue(object value, string action)
        {
            if (value == null)
        ...
        #region ctor
        public AckStream(NetStream stream)
        {
            stream.WriteTo(this);
        }
        public AckStream(MessageState state, string message)
        {
            ...
            WriteAck(ack, ack.GetType().FullName, state, message);
        ...
        public void ReadAck(TransformType returnType)
        {
            IBinaryStreamer streamer = new BinaryStreamer(this);
            this.Position = 0;
            //int count = streamer.ReadValue<int>();
